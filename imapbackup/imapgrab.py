...
import os
import subprocess
from imap_utf7 import decode as imap_utf7_decode
...
def imapgrab():
    ig_cmd = OptionParser(
        ...
        description="Description: ImapGrab connects to a imap server and downloads mail from selected mailboxes to "
                    "mbox files. Released under GPLv2.")
    ...
    ig_cmd.add_option("-m", "--mailboxes", dest="mailboxes",
                      help="Comma separated list of mailboxes to download (i.e. \"Box1, Box2, Box3\") ('{,}' for "
                           "non-separating commas) ('_ALL_' for all mailboxes, '-Box1' to except Box1 from _ALL_) "
                           "(required for -d)")
    ig_cmd.add_option("-f", "--folder", dest="folder",
                      help="Path to folder (optional, creates imapgrab folder in current directory as default)")
    ig_cmd.add_option("-L", "--localuser", dest="localuser",
                      help="User that writes to the mailboxes. (only use when invoking imapgrab as root)")
    ...
    if ig_options.debug:
        print("DEBUG_000: parsed options:\n", ig_options, "\nDEBUG: parsed arguments:\n",
              ig_args)

    # verbose shows title
    if not ig_options.quiet and not ig_options.version and not ig_options.about:
        print("IMAP Grab " + ig_version_short + "\n---")

    # show version number
    if ig_options.version:
        if ig_options.debug:
            print("DEBUG_001: printing version number")
        if ig_options.verbose:
            print("Printing version number:")
        print(ig_version_long)

    # show about display
    elif ig_options.about:
        if ig_options.debug:
            print("DEBUG_002: printing about display")
        if ig_options.verbose:
            print("Printing about display:")
        print(ig_about)

    # check for list or download
    elif ig_options.list or ig_options.download:

        # ERROR if both list and download are used
        if ig_options.list and ig_options.download:
            if ig_options.debug:
                print("DEBUG_003: both -l and -d were parsed")
            if ig_options.verbose:
                print("Both -l and -d were parsed")
            print(
                "ERROR: Cannot list mailboxes (--list or -l) and download backup (--download or -d) at the same time.")

        # ERROR if username/password/server
        elif ig_options.username is None:
            print("ERROR: No username set (--username or -u)")
        elif ig_options.password is None:
            print("ERROR: No password set (--password or -p)")
        elif ig_options.server is None:
            print("ERROR: No server set (--server or -s)")

        # --list, call list function
        elif ig_options.list:
            if ig_options.debug:
                print("DEBUG_004: -l parsed, calling list function")
            if ig_options.verbose:
                print("List option selected")
            ig_list = ig_list_mailboxes(ig_options)
            if not ig_options.quiet:
                print("List of mailboxes available:")
            for i in ig_list:
                print(i)

        # --download, call download function
        elif ig_options.download:
            if ig_options.debug:
                print("DEBUG_005: -d parsed, calling download function")
            if ig_options.verbose:
    ...
    # ERROR and show help
        ...
        if ig_options.debug:
            print("DEBUG_006: no options entered")
...
def ig_list_mailboxes(ig_options):
    ...
    if ig_options.ssl:
        if ig_options.debug:
            print("DEBUG_007: -S parsed, connecting via SSL")
        if ig_options.verbose:
            print("Connecting to IMAP server via SSL")
        ...
        if ig_options.debug:
            print("DEBUG_008: no -S parsed, connecting with no SSL")
        if ig_options.verbose:
    ...
    if ig_options.debug:
        print("DEBUG_010: sending username/password to server")
    if ig_options.verbose:
        print("Logging into IMAP server")
    ig_imap.login(ig_options.username, ig_options.password)
    if ig_options.debug:
        print("DEBUG_011: connected to IMAP server")
    ...
    imap_list_str = [imap_utf7_decode(x) for x in imap_list[1]]
    imap_list = (imap_list[0], imap_list_str)
    ...
    ig_imap.logout()
    if ig_options.debug:
        print("DEBUG_013: logged out of server")
    return ig_list
...
def ig_backup_mail(ig_options):
    ...
    if ig_options.debug:
        print("DEBUG_014: getting list of mailboxes:")
    ig_list = ig_list_mailboxes(ig_list_options)
    if ig_options.debug:
        print(ig_list)

    # Set default port
    if ig_options.port is None and ig_options.ssl:
        ig_options.port = "993"
    ...
    if ig_options.debug:
        print("DEBUG_015: removed non-separating commas: \"" + ig_download_str_rm_comma + "\"")
    ...
    if ig_options.debug:
        ...
        print(ig_download_list)
    if ig_options.debug:
        print("DEBUG_018: checking -m list against available mailboxes on server")
    if ig_options.verbose:
        print("Checking available mailboxes on server")
    ig_all_found = []
    for i in ig_download_list:
        for n in ig_list:
            if i == n:
    ...
    if len(ig_all_found) != len(ig_download_list):
        ...
        print("ERROR: Some of the mailboxes you selected are not available for download. Use list option "
              "(-l or --list) to see availabe mailboxes.")
        return
    if ig_options.debug:
        print("DEBUG_019: found all items in -m")

    # Create directory for storage
    if ig_options.folder is None:
        ig_options.folder = os.getcwd() + os.sep + "imapgrab"
        if ig_options.debug:
            print("DEBUG_020: no -f parsed, using current directory: " + ig_options.folder + os.sep + "imapgrab")
    ...
    if not os.path.exists(ig_options.folder):
        # Make folder path from root
        if ig_options.folder[0] is not os.sep:
            ig_options.folder = os.getcwd() + os.sep + ig_options.folder
        if ig_options.debug:
            print("DEBUG_021: folder does not exist, creating new folder in " + ig_options.folder)
        if ig_options.verbose:
            print("Creating folder for backup")
        os.makedirs(ig_options.folder)
    else:
        if ig_options.debug:
            print("DEBUG_022: directory already exists")

    # Create rc files for getmail
    if ig_options.debug:
        print("DEBUG_023: creating rc files for getmail")
    if ig_options.verbose:
        print("Creating download config files in " + ig_options.folder)
    for i in ig_download_list:
        ...
        if os.sep in i:
            ...
            for f in range(0, len(folders) - 1):
                ...
                if os.path.exists(fpath) is not True:
                    if ig_options.debug:
                        print("DEBUG_024: creating \"" + folders[f] + "\" folder since it doesn't exist")
                    if ig_options.verbose:
                        print("\"" + folders[f] + "\" folder doesn't exist, creating folder")
                    os.mkdir(fpath)
        else:
            fpath = ig_options.folder
            filename = i
        ...
        if ig_options.verbose:
            print("Created \"" + fpath + os.sep + filename + ".rc\"")
        if ig_options.debug:
        ...
        if ig_options.maildir:
            ...
            for f in maildir_directories:
                if os.path.isdir(fpath + os.sep + filename + os.sep + f) is not True:
                    if ig_options.verbose:
                        print("Maildir directory doesn't exist, creating \""
                              + fpath + os.sep + filename + os.sep + f + "\"")
                    if ig_options.debug:
                        print("DEBUG_025: \"" + fpath + os.sep + filename + os.sep + f + "\" doesn't exist, creating")
                    os.mkdir(fpath + os.sep + filename + os.sep + f)
                else:
                    if ig_options.verbose:
                        print("Maildir directory already exists, using \""
                              + fpath + os.sep + filename + os.sep + f + "\"")
                    if ig_options.debug:
                        print("DEBUG_026: \"" + fpath + os.sep + filename + os.sep + f + "\" exists")
        else:
            # Create mbox file if doesn't exist
            if os.path.isfile(fpath + os.sep + filename + ".mbox") is not True:
                if ig_options.verbose:
                    print("Mbox file doesn't exist, creating \"" + fpath + os.sep + filename + ".mbox\"")
                if ig_options.debug:
                    print("DEBUG_025: \"" + fpath + os.sep + filename + ".mbox\" doesn't exist")
                open(fpath + os.sep + filename + ".mbox", "w").close()
            else:
                if ig_options.verbose:
                    print("Mbox file already exists, using \"" + fpath + os.sep + filename + ".mbox\"")
                if ig_options.debug:
                    print("DEBUG_026: \"" + fpath + os.sep + filename + ".mbox\" exists")
    ...
    for mailbox in ig_download_list:
        ...
        # Call getmail command
        if ig_options.verbose:
            print("Calling getmail to retrieve mail for mailbox \""
                  + ig_options.folder + os.sep + path_from_folder + os.sep + filename + "\"")
        if not ig_options.quiet:
            print("Downloading mailbox \"" + mailbox + "\"")
        if ig_options.debug:
            print("DEBUG_027: calling \"" + cmd + " -g " + ig_options.folder + " -r " + rc_file_loc + "\"")
        if ig_options.verbose or ig_options.debug:
