...
from imap_utf7 import encode as imap_utf7_encode
...
if sys.version_info < (3, 0):
    print("IMAP Upload requires Python 3.0 or later.", file=sys.stderr)
    sys.exit(1)
...
class MyOptionParser(OptionParser):
    ...
    def set_time_fields(self, option, opt_str, value, parser):
        ...
        if set(fields) - {"from", "received", "date"}:
...
class Progress:
    ...
    def begin(self, msg):
        ...
        size, prefix = si_prefix(float(len(msg.as_bytes())), threshold=0.8)
        sbj = self.decode_subject(msg["subject"] or "")
        print(self.format % (self.count + 1, size, prefix + "B", left_fit_width(sbj, 30)), end=' ', file=sys.stderr)
    ...
    def end_ok(self):
        ...
        self.count += 1
        self.ok_count += 1
        print("OK (%d sec)" % math.ceil(time.time() - self.time_began), file=sys.stderr)

    def end_ng(self, err):
        """Called when an error has occurred while processing a message."""
        print("NG (%s)" % err, file=sys.stderr)

    def end_all(self):
        """Called when all message was processed."""
        print("Done. (OK: %d, NG: %d)" % (self.ok_count, self.total_count - self.ok_count), file=sys.stderr)


def upload(imap, box, src, err, time_fields):
    print("Uploading to {}...".format(box), file=sys.stderr)
    print("Counting the mailbox (it could take a while for the large one).", file=sys.stderr)
    p = Progress(len(src))
...
def recursive_upload(imap, box, src, err, time_fields):
    for file in os.listdir(src):
        ...
        if os.path.isdir(path):
            ...
            print("Found mailbox at {}...".format(path), file=sys.stderr)
...
class IMAPUploader:
    ...
    def upload(self, box, delivery_time, message, retry=None):
        ...
        try:
            ...
            if retry == 0:
                raise
        print("(Reconnect)", end=' ', file=sys.stderr)
        time.sleep(5)
    ...
    def open(self):
        ...
        try:
            ...
            print("(create error: )" + str(e), file=sys.stderr)
...
def main(args=None):
    try:
        ...
        # Surround with quotes to allow spaces in box name
        options.box = '"{}"'.format(options.box)
        ...
        print("Connecting to %s:%s." % (options["host"], options["port"]), file=sys.stderr)
        ...
        print("No such mailbox:", e, file=sys.stderr)
        return 1
    except socket.timeout:
        print("Timed out", file=sys.stderr)
        return 1
    except imaplib.IMAP4.error as e:
        print("IMAP4 error:", e, file=sys.stderr)
        return 1
    except KeyboardInterrupt:
        print("Interrupted", file=sys.stderr)
        return 130
    # except Exception as e:
    #     print("An unknown error has occurred: ", e, file=sys.stderr)
    #     return 1
